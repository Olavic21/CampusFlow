...
class User(Base):
    ...
    # RBAC minimal : student (défaut) | staff | admin
...
class FavoriteRoute(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "start_location_id", "end_location_id", name="uq_fav_route"
        ),
    )
...
class Incident(Base):
    """
    Incident / fermeture de zone sur le campus (Phase 2 — Smart Mobility).

    Un incident actif sur un bâtiment est évité par le calcul d'itinéraire
    et signalé sur la carte et les fiches.
    """
    __tablename__ = "incidents"

    id          = Column(Integer, primary_key=True, index=True)
    location_id = Column(Integer, ForeignKey("locations.id"), nullable=False, index=True)
    type        = Column(String(50), nullable=False, default="closure")   # closure | works | event
    severity    = Column(String(20), nullable=False, default="high")      # low | medium | high
